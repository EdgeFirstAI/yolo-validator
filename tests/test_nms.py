...
def test_nms_cv2_suppresses_overlap():
    from yolo_validator.nms import nms_class_aware_cv2
    boxes = np.array([[0, 0, 10, 10], [1, 1, 11, 11], [100, 100, 110, 110]], dtype=np.float32)
    scores = np.array([0.9, 0.8, 0.7], dtype=np.float32)
    classes = np.array([0, 0, 0])
    keep = nms_class_aware_cv2(boxes, scores, classes, conf=0.001, iou_thres=0.5, max_det=300)
    assert sorted(keep.tolist()) == [0, 2]   # box 1 suppressed by box 0


def test_nms_cv2_keeps_overlapping_diff_classes():
    from yolo_validator.nms import nms_class_aware_cv2
    boxes = np.array([[0, 0, 10, 10], [0, 0, 10, 10]], dtype=np.float32)
    scores = np.array([0.9, 0.8], dtype=np.float32)
...
def test_nms_cv2_empty():
    from yolo_validator.nms import nms_class_aware_cv2
    keep = nms_class_aware_cv2(np.zeros((0, 4), np.float32), np.zeros((0,), np.float32),
                               np.zeros((0,), np.int64), conf=0.001, iou_thres=0.5, max_det=300)
    assert keep.shape == (0,)
